...
Blacklight.configure(:shared) do |config|
  ...
  config[:search_fields] << {:key => :title, :display_label => 'Title', :qt => 'title_search'}
  config[:search_fields] << {:key => :author, :display_label =>'Author', :qt => 'author_search'}
  config[:search_fields] << {:key => :subject, :display_label => 'Subject', :qt=> 'subject_search'}
