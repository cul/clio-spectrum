...
describe 'collection output tests', js: true do
  ...
  it 'test condensed holdings output full' do
    ...
    within ('div#clio_holdings') do
      expect(page).to have_text('Butler Stacks (Enter at the Butler Circulation Desk)', wait: 5)
    end

    visit catalog_path('1052500')
    within ('div#clio_holdings') do
      expect(page).to have_text('MICROFLM FN 41 Library has: v.1851:Sept.-2003:Dec.14, 2004:Jan.', wait: 5)
    end
