module Spectrum
  class SearchBuilder < Blacklight::Solr::SearchBuilder
      ...
      # 
      # moved over from local_solr_helper_extension.rb, to try to fix this:
      #   DEPRECATION WARNING: Building search parameters by calling 
      #   add_facet_fq_to_solr on Spectrum::SearchEngines::Solr. 
      #   This behavior will be deprecated in Blacklight 6.0. Instead, 
      #   define add_facet_fq_to_solr on a subclass of Spectrum::SearchBuilder 
      #   and set search_builder_class in the configuration.
      # (but doesn't work...)
      # 
    ...
    # OVERRIDE Blacklight::Solr::SearchBuilder#facet_value_to_fq_string(), to
    # support excluded values (negative facets) and choice of operator (AND/OR)
    ...
    def add_advanced_search_to_solr(solr_parameters)
      if is_advanced_search?()
        ...
        solr_parameters[:q] = advanced_q.join(" #{advanced_search_operator()} ")
      end
    end
    ...
    def is_advanced_search?()
      blacklight_params[:search_field] == 'advanced' && blacklight_params[:adv].kind_of?(Hash)
    end

    def advanced_search_operator()
      advanced_operator = blacklight_params[:advanced_operator] || 'AND'
    end
