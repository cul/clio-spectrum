...
(function() {
  $(function() {
    ...
    return $('.advanced_search_toggle').click(function() {
      var parent;
      parent = $(this).parents('.search_boxes');
      parent.find('.advanced_search .search_box').toggle();
      return parent.find('.basic_search .search_box').toggle();
    });
  });
...
(function() {
  $(function() {
    return $('.search_option_action').click(function() {
      $('.busy').show();
...
(function() {
  $(function() {
    return $('.result_set.async_result_set').each(function() {
      var result_set, url;
      result_set = $(this);
      ...
      return $.get(url, function(data) {
        ...
        return async_lookup_item_details(result_set);
      });
    });
  });
...
(function() {
  $(function() {
    ...
    $(document).on('click', '.toggle-teaser', function() {
    ...
    $(".viewstyle_link").click(function() {
      var request, viewstyle;
      $('.busy').show();
      viewstyle = $(this).attr('viewstyle');
      request = $.get('/set_browser_option?name=viewstyle&value=' + viewstyle, {
        cache: false
      });
      ...
      return request.always(function(data_or_jqXHR, textStatus, jqXHR_or_errorThrown) {});
    });
    $('.toggle_all.contract').parents("#facets").find('.range_limit').show();
    $('.toggle_all.contract').parents("#facets").find('ul').show();
    $('.toggle_all').click(function(e) {
      var expand, facets;
      e.preventDefault();
      ...
      if (expand) {
        ...
        return $('.expand_all_option').removeClass('hide').show();
      } else {
    ...
    $('.toggle_expand_all_option').click(function() {
      if ($(this).is(':checked')) {
        return $.get('/set_browser_option?name=always_expand_facets&value=true');
      } else {
        return $.get('/set_browser_option?name=always_expand_facets&value=false');
      }
    });
    return $('.collapse-toggle.panel-heading').click(function() {
      var facet_tag;
      facet_tag = $(this).data("facet-tag");
      if ($(this).hasClass('collapsed')) {
        return $.get('/set_browser_option?name=' + facet_tag + '&value=open');
      } else {
        return $.get('/set_browser_option?name=' + facet_tag + '&value=closed');
      }
    });
  });
...
(function() {
  $(function() {
    $("ul#datasources li, #toolbar-container .box").hover(function() {
      return $(this).addClass('hover');
    }, function() {
      return $(this).removeClass('hover');
    });
    $("#datasource_expand").click(function() {
      $(this).hide();
      return $("#expanded_datasources").slideDown();
    });
    $("#datasource_contract").click(function() {
      return $("#expanded_datasources").slideUp(100, function() {
        return $("#datasource_expand").show();
      });
    });
    return $("#mobile_datasource_select").change(function() {
      var datasource_href, select;
      select = '#datasources li[source="' + $(this).val() + '"] a';
      datasource_href = $(select).attr('href');
      if (datasource_href !== '#') {
        $('.busy').show();
        return window.location = datasource_href;
      }
    });
  });
...
(function() {
  $(function() {
    return async_lookup_item_details($('#outer-container'));
  });

  this.async_lookup_item_details = function(element) {
    var catalog_items, fedora_items, standard_id_sets;
    fedora_items = [];
    catalog_items = [];
    standard_id_sets = [];
    $(element).find('.result').each(function() {
      var item, res, source, standard_id_set_csv;
      res = $(this);
      source = res.attr('source');
      item = res.attr('item_id');
      if (source === 'academic_commons') {
        return fedora_items.push(item);
        ...
        if (standard_id_set_csv) {
          return standard_id_sets.push(standard_id_set_csv);
        }
      }
    });
    ...
    if (standard_id_sets.length) {
      return retrieve_google_jackets(standard_id_sets);
    }
  };

  this.load_clio_holdings = function(id) {
    $("span.holding_spinner").show;
    $("#clio_holdings .holdings_error").hide;
    return $.ajax({
      url: '/backend/holdings/' + id,
      success: function(data) {
        return $('#clio_holdings').html(data);
      },
      error: function(data) {
        $("span.holding_spinner").hide();
        return $('#clio_holdings .holdings_error').show();
      }
    });
  };

  this.load_hathi_holdings = function(id) {
    ...
    return $.ajax({
      ...
      success: function(data) {
        ...
        $('#hathi_holdings').show();
        return $(".expander").click(function() {
          $('#hathi_holdings').find(".expander").hide();
          return $('#hathi_holdings').find(".expander_more").removeClass('expander_more');
        });
      },
      error: function(data) {
        $(".hathi_holdings_check").hide();
        $('.hathi_holdings_error').show();
        return $('#hathi_holdings').show();
      }
    });
  };

  this.retrieve_fedora_resources = function(fedora_ids) {
    var url;
    url = clio_backend_url + '/fedora/resources/' + fedora_ids.join('/');
    return $.getJSON(url, function(data) {
      var extension, fedora_id, fedora_selector, filename, first_resource, format_icons, i, icon, resource, resources, results, txt;
      results = [];
      for (fedora_id in data) {
        resources = data[fedora_id];
        ...
        if (first_resource) {
          results.push($(fedora_selector).html('No downloads found for this item.'));
        } else {
          results.push(void 0);
        }
      }
  ...
  this.retrieve_holdings = function(bibids) {
    var url;
    url = clio_backend_url + '/holdings/status/' + bibids.join('/');
    return $.getJSON(url, function(data) {
      var bib, holding_id, holdings, results, selector, status, status_upcase;
      results = [];
      for (bib in data) {
        holdings = data[bib];
        results.push((function() {
          var ref, results1;
          ref = data[bib].statuses;
          results1 = [];
          for (holding_id in ref) {
            status = ref[holding_id];
            ...
            results1.push($(selector).attr("alt", status_upcase));
          }
          return results1;
        })());
      }
  ...
  this.update_google_holdings = function(bibkeys, data) {
    var bibkey, bibkey_data, bibkey_name, find_id, gbs_cover, index, results, search_form, selector, strip_querystring;
    results = [];
    for (index in bibkeys) {
      ...
      if (selector.length > 0 && bibkey_data) {
        ...
        if (bibkey_data.preview !== "noview") {
          ...
          if (bibkey_data.preview === "partial") {
            gbs_cover.find(".gbs_preview_partial").show();
          }
          if (bibkey_data.preview === "full") {
            results.push(gbs_cover.find(".gbs_preview_full").show());
          } else {
            results.push(void 0);
          }
        } else {
  ...
  this.retrieve_google_jackets = function(standard_id_sets) {
    var j, len, results, standard_id_set_csv;
    results = [];
    for (j = 0, len = standard_id_sets.length; j < len; j++) {
      standard_id_set_csv = standard_id_sets[j];
      results.push(retrieve_google_jacket_for_single_item_v2(standard_id_set_csv));
    }
    return results;
  };

  this.retrieve_google_jacket_for_single_item_v2 = function(standard_id_set_csv) {
    var api_url, books_url, tag;
  ...
  this.google_books_response_callback = function(data) {
    var id, id_as_class, value_hash;
    for (id in data) {
      value_hash = data[id];
...
(function() {
  var bind_alert_management_buttons, bind_alert_management_form, bind_datepickers;

  bind_datepickers = function() {
    return $("input[data-datepicker-format]").datepicker({
      weekStart: 1,
      days: ["S", "M", "T", "W", "T", "F", "S"],
  ...
  bind_alert_management_buttons = function(alert_div) {
    $(alert_div).find('.edit-item').click(function() {
      var button, row;
      row = $(this).parents('tr');
      button = $(this);
      return $.ajax({
        ...
        dataType: 'script',
          ...
          if (jqXHR.statusText === "OK") {
            ...
            return new_row.find('div.edit_form').fadeIn();
          }
        }
      });
    });
    return $(alert_div).find('.delete-item').click(function() {
      var button;
  ...
  bind_alert_management_form = function(form) {
    return $('#show_alert_management form').submit(function() {
      var parent_div, parent_row;
      form = $(this);
      ...
      $.ajax({
        ...
        dataType: 'json',
          ...
          var item, item_id, item_id_selector, response, row;
          ...
          return $.get('/item_alerts/' + item_id + '/show_table_row', function(data) {
            var add_row;
            parent_div.fadeOut();
        ...
        fail: function(data, textStatus, jqXHR) {
          var alert_html, alerts, error, errors, field, i, len, response;
          ...
          alert_html += "</ul></div>";
          return form.find('.alerts').html(alert_html);
        }
      });
      return false;
    });
  ...
  $(function() {
    $('#show_alert_management .add_row_button').click(function() {
      $('#create_alert').fadeIn();
      return $(this).parents('tr').hide();
    });
    bind_alert_management_buttons('#show_alert_management');
    bind_alert_management_form($('#show_alert_management form'));
    return bind_datepickers();
  });
...
(function() {
  $(function() {
    $('.hide_mini_browse').click(function() {
      $('#mini_browse_list').hide();
      $('.hide_mini_browse').toggleClass('disabled');
      return $('.show_mini_browse').toggleClass('disabled');
    });
    $('.show_mini_browse').click(function() {
      var active_bib, active_shelfkey;
    ...
    return $('.call_number_toggle').click(function() {
      var clicked_bib, clicked_shelfkey;
      ...
      return $('.full_screen_link').attr("href", "/browse/shelfkey_full/" + encodeURIComponent(clicked_shelfkey));
    });
  });

  this.install_toolbar_click_handlers = function() {
    $('#mini_browse_list .call_number_nav_previous').click(function() {
      var firstShelfkey;
      firstShelfkey = $(this).data('firstshelfkey');
      load_shelfkey(firstShelfkey, 0, 9);
      return false;
    });
    $('#mini_browse_list .call_number_nav_next').click(function() {
      var lastShelfkey;
      lastShelfkey = $(this).data('lastshelfkey');
      load_shelfkey(lastShelfkey, 0, 0);
      return false;
    });
    return $('#mini_browse_list .call_number_jump').click(function() {
      var jump_shelfkey;
  ...
  this.load_shelfkey = function(shelfkey, bib, before_count) {
    var mini_browse_url;
    ...
    return $.ajax({
      ...
      error: function(data) {
        ...
        return $("#nearby .nearby_content").hide();
      }
    });
  };
...
(function() {
  $(function() {
    $('.saved_list_add').click(function() {
      var date, expires, form, identifier, items_to_add;
    ...
    $('.item_select_checkbox').change(function() {
      return $(this).parents(".result.document").toggleClass('selected_item', this.checked);
    });
    return $('.item_select_checkbox').click(function() {
      var identifier;
      identifier = $(this).data("identifier");
      if (typeof identifier === "undefined") {
      ...
      if (this.checked) {
        return setSelectedItems("add", identifier);
      } else {
        return setSelectedItems("remove", identifier);
      }
    });
  });

  this.setSelectedItems = function(verb, id_param) {
    var request;
    request = $.post("/selected_items", {
      verb: verb,
      id_param: id_param
    });
    return request.fail(function(jqXHR, textStatus, errorThrown) {
      return flashMessage("danger", "Select failed: " + errorThrown + "  " + jqXHR.responseText);
    });
  };

  this.resetSelectedItemsList = function() {
    var item_key_list;
    item_key_list = getSelectedItemKeyList();
    if (item_key_list.length > 0) {
      return setSelectedItems("reset", item_key_list);
    }
  };
  ...
  this.deselectAll = function() {
    ...
    return setSelectedItems("clear");
  };

  this.getSelectedItemKeyList = function() {
    var item_key_array;
    item_key_array = $(".result.document").has('.item_select_checkbox:checked').map(function() {
      return $(this).attr('item_id');
    });
    return item_key_array.get();
  };

  this.appendSelectedToURL = function(a_element) {
    var _href, item_key_list, item_key_param_list, item_keys_as_params;
    item_key_list = getSelectedItemKeyList();
    if (item_key_list.length === 0) {
      flashMessage("notice", "No items selected");
      return false;
    }
    item_keys_as_params = item_key_list.map(function(item_key) {
      return "id[]=" + item_key;
    });
  ...
  this.saveSelectedToNamedList = function(name) {
    var item_key_list;
    item_key_list = getSelectedItemKeyList();
    if (item_key_list.length === 0) {
      flashMessage("notice", "No items selected");
    ...
    return saveItemListToNamedList(item_key_list, name);
  };

  this.saveBibToNamedList = function(bib, name) {
    var item_key_list;
    item_key_list = [bib];
    return saveItemListToNamedList(item_key_list, name);
  };

  this.saveItemListToNamedList = function(item_key_list, name) {
    var request;
    request = $.post('/lists/add', {
      item_key_list: item_key_list,
      name: name
    });
    request.done(function(data) {
      return flashMessage("success", data);
    });
    return request.fail(function(jqXHR, textStatus, errorThrown) {
      return flashMessage("danger", "Save failed: " + errorThrown + "  " + jqXHR.responseText);
    });
  };

  this.moveSelectedToNamedList = function(savedlist_move_path, from_owner, from_list, to_list) {
    var full_move_url, item_key_list;
    item_key_list = getSelectedItemKeyList();
    if (item_key_list.length === 0) {
      flashMessage("notice", "No items selected");
      return false;
    }
    full_move_url = savedlist_move_path + "?from_owner=" + from_owner + "&from_list=" + encodeURIComponent(from_list) + "&to_list=" + encodeURIComponent(to_list) + "&" + $.param({
      'item_key_list': item_key_list
    });
    return window.location.href = full_move_url;
  };

  this.copySelectedToNamedList = function(savedlist_copy_path, from_owner, from_list, to_list) {
    var full_copy_url, item_key_list;
    item_key_list = getSelectedItemKeyList();
    if (item_key_list.length === 0) {
      flashMessage("notice", "No items selected");
      return false;
    }
    full_copy_url = savedlist_copy_path + "?from_owner=" + from_owner + "&from_list=" + encodeURIComponent(from_list) + "&to_list=" + encodeURIComponent(to_list) + "&" + $.param({
      'item_key_list': item_key_list
    });
    return window.location.href = full_copy_url;
  };

  this.removeSelectedFromList = function(savedlist_remove_path, list_id) {
    var full_remove_url, item_key_list;
    item_key_list = getSelectedItemKeyList();
    if (item_key_list.length === 0) {
      flashMessage("notice", "No items selected");
      return false;
    }
    full_remove_url = savedlist_remove_path + "?list_id=" + list_id + "&" + $.param({
      'item_key_list': item_key_list
    });
    return window.location.href = full_remove_url;
  };

  this.flashMessage = function(status, message) {
    var close_button, flash_html_content;
    if (status === 'notice') {
      status = 'info';
    }
    if (status = 'error') {
    ...
    return $("div#main-flashes div.flash_messages").append(flash_html_content);
  };
...
(function() {
  $(function() {
    $('.link_back').click(function() {
      ...
      if ($(this).data('bib')) {
        return window.localStorage['highlight_bib'] = $(this).data('bib');
      }
    });
    if ($('.scroll_memory').length) {
      $(document).scroll(function() {
        return window.localStorage['scroll'] = $(document).scrollTop();
      });
      ...
      if (typeof window.localStorage['highlight_bib'] !== 'undefined') {
        return localStorage.removeItem('highlight_bib');
      }
    }
  });
